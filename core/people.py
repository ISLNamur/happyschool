...
class People:
    ...
    def get_person(self, teaching: list = ("all",), person_type=ALL, **kwargs) -> object:
        """
    ...
    @staticmethod
    def get_teacher_by_id(person_id: int, teaching: list = ("all",)) -> object:
        """
        ...
        try:
            if "all" in teaching:
                return ResponsibleModel.objects.get(matricule=person_id, is_teacher=True)

            return ResponsibleModel.objects.get(
                matricule=person_id, teaching__name__in=teaching, is_teacher=True
            )
        except ObjectDoesNotExist:
            return None

    @staticmethod
    def get_educator_by_id(person_id: int, teaching: list = ("all",)) -> object:
        """
        ...
        try:
            if "all" in teaching:
                return ResponsibleModel.objects.get(matricule=person_id, is_educator=True)

            return ResponsibleModel.objects.get(
                matricule=person_id, teaching__name__in=teaching, is_educator=True
            )
        except ObjectDoesNotExist:
            return None

    @staticmethod
    def get_student_by_id(person_id: int, teaching: list = ("all",)) -> object:
        """
    ...
    @staticmethod
    def get_responsible_by_id(person_id: int, teaching: list = ("all",)) -> object:
        """
    ...
    def get_people_by_name(
        ...
        students = self._get_people_by_name_by_model(
            StudentModel, name=name, teaching=teaching, active=active
        )
        responsibles = self._get_people_by_name_by_model(
            ResponsibleModel, name=name, teaching=teaching, active=active
        )

        return {"student": students, "responsible": responsibles}

    @staticmethod
    def _get_people_by_name_by_model(
        model_name: Model,
        name: str,
        teaching: list = ("all",),
        active: bool = True,
    ...
    def get_teachers_by_name(
        self, name: str, teaching: list = ("all",), active: bool = True
    ) -> QuerySet:
        """
        ...
        :return: A QuerySet of teachers.
        """
        return self._get_people_by_name_by_model(
            ResponsibleModel,
            name=name,
            teaching=teaching,
            active=active,
            additional_filter={"is_teacher": True},
        )[0]

    def get_educators_by_name(
        self, name: str, teaching: list = ("all",), active: bool = True
    ) -> QuerySet:
        """
        ...
        return self._get_people_by_name_by_model(
            ...
            teaching=teaching,
            active=active,
            additional_filter={"is_educator": True},
        )[0]

    def get_responsibles_by_name(
        self, name: str, teaching: list = ("all",), active: bool = True
    ) -> QuerySet:
        """
    ...
    def get_students_by_classe(classe: object, teaching: list = ("all",)) -> QuerySet:
        ...
        # classe is a string.
        if classe[0].isdigit():
            students = students.filter(classe__year=int(classe[0])).order_by(
...
def get_classes(
    ...
    educ_by_years: Union[bool, str] = True,
) -> QuerySet:
    ...
    :param educ_by_years: True, False or "both". If true, get classes by year access. Otherwise get it by classes, defaults to True
    :type educ_by_years: bool or str, optional
    ...
    if check_access and user:
        ...
        if user.groups.filter(
            name__in=[settings.SYSADMIN_GROUP, settings.DIRECTION_GROUP, settings.PMS_GROUP]
        ).exists():
            ...
            if educ_by_years:
                ...
                if educ_by_years == "both":
                    classes = classes.union(
                        responsible.classe.all().filter(teaching__in=teaching_models)
                    )
            else:
        ...
        if tenure_class_only:
            ...
            return (
                responsible.classe.all()
                .filter(teaching__in=teaching_models)
                .union(responsible.tenure.all().filter(teaching__in=teaching_models))
            )
    else:
...
def check_access_to_student(
    student: StudentModel, user: User, tenure_class_only: bool = True, educ_by_years: bool = True
) -> bool:
...
def get_teachers_from_student(stud: StudentModel) -> QuerySet:
    ...
    if stud_teach_rel == CoreSettingsModel.BY_CLASSES:
        return ResponsibleModel.objects.filter(Q(classe=stud.classe) | Q(tenure=stud.classe))
