...
transformé pour être lisible et alléger pour le navigateur client.
...
```
Ensuite récupérer les fichiers statiques (css,…) utilisés par django et ses
applications :
```
...
```
*Supervisor* se configure avec le fichier `/etc/supervisord.conf` :
 ```
unix_http_server]
...
server {
        ...
        location /favicon.ico {
                 alias /home/isln-user/happyschool/static/favicon.ico; # Mettre le chemin correct.
        }
...
Happyschool que ce soit pour l'envoi automatique des courriels ou pour choix des
applications.
