...
class DossierEleveSettingsModel(models.Model):
    ...
    all_access = models.ManyToManyField(
        Group,
        default=None,
        blank=True,
    ...
    filter_teacher_entries_by_tenure = models.BooleanField(default=False, help_text="Déprécié.")
...
class SanctionStatisticsModel(models.Model):
    ...
    sanctions_decisions = models.ManyToManyField(
        SanctionDecisionDisciplinaire, blank=True, default=None
    )
...
class CasEleve(models.Model):
    ...
    sanction_decision = models.ForeignKey(
        SanctionDecisionDisciplinaire, null=True, blank=True, on_delete=models.SET_NULL
    )
    ...
    datetime_sanction = models.DateTimeField(
        "date de la sanction", null=True, blank=True
    )  # Deprecated
