        ...
        <BOverlay :show="sending">
            <BRow>
                <BCol sm="4">
                    <div>
                        <BImg
                            rounded
                            :src="'/static/photos/' + form.student_id + '.jpg'"
                            fluid
            ...
            <BRow>
                <BCol class="text-end">
                    <BButton
                        variant="primary"
                        :disabled="!form.sanction_decision_id"
                        @click="askSanction"
                    >
                        Soumettre
                    </BButton>
...
export default {
    ...
    watch: {
        errors: function (newErrors) {
            let inputs = ["name", "sanction_decision_id", "demandeur", "explication_commentaire",
                "datetime_conseil", "date_sanction_end", "date_sanction", "time_sanction_start", "time_sanction_end"];
            for (let u in inputs) {
    ...
    methods: {
        ...
        askSanction: function () {
            ...
            // Send data.
            let path = `/dossier_eleve/api/ask_sanctions/${this.id > 0 ? this.id + "/" : ""}`;
            const send = this.id > 0 ? axios.put(path, data, token) : axios.post(path, data, token);
            send.then(() => {
                app.errors = {};
            ...
            axios.post("/annuaire/api/people/", data, token)
                .then((response) => {
                    ...
                    const options = response.data.map((p) => {
                        ...
                        let entry = `${p.last_name} ${p.first_name}`;
                        if ("is_secretary" in p) {
                            ...
                            // It's a student.
                            entry = { display: p.last_name + " " + p.first_name, matricule: p.matricule };
